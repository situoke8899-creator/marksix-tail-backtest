...
const FREEZE_VERSION = 'macau-prediction-freeze-v4'

function freezeKey(expect){
  return `macaujc-freeze-${FREEZE_VERSION}-${expect}`
}

function readFreeze(expect){
  if(typeof window==='undefined'||!expect) return null
  try{
    const raw=window.localStorage.getItem(freezeKey(expect))
    if(!raw) return null
    const value=JSON.parse(raw)
    return value?.version===FREEZE_VERSION ? value : null
  }catch{
    return null
  }
}

function writeFreeze(record){
  if(typeof window==='undefined'||!record?.expect) return record
  try{
    // 锁定规则：已经存在的记录永远不重写预测方案。
    const old=readFreeze(record.expect)
    if(old) return old
    window.localStorage.setItem(freezeKey(record.expect),JSON.stringify(record))
  }catch{}
  return record
}

function updateSettledFreeze(record,draw){
  if(!record||record.settled) return record

  const actualZodiac=draw.specialZodiac||''
  const actualHead=getHead(draw.specialNumber)
  const actualTail=getTail(draw.specialNumber)
  ...
  const settled={
    ...
    zodiac:record.zodiac.map(x=>({...x,hit:Boolean(actualZodiac)&&x.picks.includes(actualZodiac)})),
    head:record.head.map(x=>({...x,hit:actualHead>=0&&x.picks.includes(actualHead)})),
    tail:record.tail.map(x=>({...x,hit:actualTail>=0&&x.picks.includes(actualTail)})),
  }

  try{
    window.localStorage.setItem(freezeKey(record.expect),JSON.stringify(settled))
  }catch{}

  return settled
}

function createPredictionFreeze(expect,pastHistory,source='live'){
  const zPicks=allPicks(pastHistory)
  const hResults=allHeadResults(pastHistory)
  const tResults=allTailResults(pastHistory)
  ...
  return {
    ...
    head:HEAD_FORMULAS.map((f,i)=>({
      id:f.id,
      name:f.name,
      picks:[...(hResults[i]?.picks||[])],
      exclude:hResults[i]?.exclude,
      hit:null,
    })),

    tail:TAIL_FORMULAS.map((f,i)=>({
      id:f.id,
      name:f.name,
      picks:[...(tResults[i]?.picks||[])],
      exclude:[...(tResults[i]?.exclude||[])],
      hit:null,
    })),
  }
}

function syncFreezeRecords(analysisHistory,nextExpect){
  if(typeof window==='undefined'||!analysisHistory?.length) return []

  // 1. 先冻结“下一期”当前方案。以后开奖出来绝不改这个预测。
  if(nextExpect && !readFreeze(nextExpect)){
    writeFreeze(createPredictionFreeze(nextExpect,analysisHistory,'live'))
  }

  const visible=analysisHistory.slice(0,100)
  const settled=[]

  // 2. 最近100期开奖：有旧冻结就直接读取；没有的历史期做一次“严格滚动补建”。
  //    补建时只使用该期之前的数据，补建后永久锁定。
  visible.forEach((draw,index)=>{
    let record=readFreeze(draw.expect)

    if(!record){
      const past=analysisHistory.slice(index+1)
      if(past.length>=20){
        record=writeFreeze(createPredictionFreeze(draw.expect,past,'backfill'))
      }
    }

    if(record){
      record=updateSettledFreeze(record,draw)
      if(record?.settled) settled.push(record)
    }
  })
...
function frozenStat(records,type,id,size){
  const rows=records
    .slice(0,size)
    .map(r=>r?.[type]?.find(x=>x.id===id))
    .filter(x=>typeof x?.hit==='boolean')

  const results=rows.map(x=>x.hit)
  const hitCount=rows.filter(x=>x.hit).length
...
export default function Page(){
  ...
  useEffect(()=>{
    ...
    const records=syncFreezeRecords(analysisHistory,data?.nextExpect)
    ...
    <div className="wrap">
      ...
      {data?.warnings?.length>0&&(
        <div className="note" style={{marginBottom:16}}>
          <strong>数据源提示：</strong>{data.warnings.join('；')}
        </div>
      )}
      ...
      <section className="card">
        <h2>开奖记录冻结｜最近100期</h2>
        <div className="note">
          每一期在开奖前都会保存当时的9生肖、4头、8尾方案；开奖后只写入“中/未中”，预测内容不再改。
          旧历史第一次进入本版本时，会使用“该期之前的数据”补建一次，然后同样永久锁定。
          因此今天显示未中，明天刷新仍然会保持未中，不会因为新数据改变旧方案。
        </div>
        <div className="history" style={{marginTop:12}}>
          {frozenRecords.slice(0,20).map(r=><div className="draw" key={r.expect}>
            <strong>第 {r.expect} 期</strong>
            <span className="muted">{r.source==='live'?'真实开奖前冻结':'历史补建冻结'}</span>
            <div style={{marginTop:5}}>
              生肖1号：
              <b style={{color:r.zodiac?.[0]?.hit?'#4ade80':'#fb7185'}}>
                {r.zodiac?.[0]?.hit?' 中':' 未中'}
              </b>
              {' ｜ '}头1号：
              <b style={{color:r.head?.[0]?.hit?'#4ade80':'#fb7185'}}>
                {r.head?.[0]?.hit?' 中':' 未中'}
              </b>
              {' ｜ '}尾1号：
              <b style={{color:r.tail?.[0]?.hit?'#4ade80':'#fb7185'}}>
                {r.tail?.[0]?.hit?' 中':' 未中'}
              </b>
            </div>
          </div>)}
      ...
      <section className="card">
        ...
        <div className="scroll"><table>
          ...
          <tbody>{ranking.map((s,i)=>{
            const f20=frozenStat(frozenRecords,'zodiac',s.id,20)
            const f30=frozenStat(frozenRecords,'zodiac',s.id,30)
            const f50=frozenStat(frozenRecords,'zodiac',s.id,50)
            const f100=frozenStat(frozenRecords,'zodiac',s.id,100)
            return <tr key={s.id}>
            <td className="rank">{i+1}</td>
            <td><strong>{s.name}</strong></td>
            <td className="formula">{s.desc}</td>
            <td><div className="zlist">{s.picks.map(z=><Z key={z} z={z}/>)}</div></td>
            <td><span className={f20.hitRate>=75?'rate good':f20.hitRate>=65?'rate mid':'rate low'}>{pct(f20.hitRate)}</span><div className="muted">{f20.hitCount}/{f20.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f30.hitRate)}</span><div className="muted">{f30.hitCount}/{f30.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f50.hitRate)}</span><div className="muted">{f50.hitCount}/{f50.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f100.hitRate)}</span><div className="muted">{f100.hitCount}/{f100.testedCount} 冻结</div></td>
            <td>{f50.maxMiss}<div className="muted">当前 {f20.currentMiss}</div></td>
            <td><div className="dots">{f20.testedCount?frozenRecords.slice(0,20).reverse().map(r=>{
              const row=r.zodiac?.find(x=>x.id===s.id)
              return <span key={r.expect} className={row?.hit?'dot hit':'dot'} title={`第${r.expect}期｜${row?.hit?'中':'未中'}｜冻结`}/>
            }):null}</div></td>
          </tr>})}</tbody>
        </table></div>
      </section>
      ...
      <section className="card">
        ...
        <div className="scroll"><table>
          ...
          <tbody>{tRanking.map((s,i)=>{
            const f20=frozenStat(frozenRecords,'tail',s.id,20)
            const f30=frozenStat(frozenRecords,'tail',s.id,30)
            const f50=frozenStat(frozenRecords,'tail',s.id,50)
            const f100=frozenStat(frozenRecords,'tail',s.id,100)
            return <tr key={s.id}>
            <td className="rank">{i+1}</td>
            <td><strong>{s.name}</strong></td>
            ...
            <td><span className={f20.hitRate>=82?'rate good':f20.hitRate>=76?'rate mid':'rate low'}>{pct(f20.hitRate)}</span><div className="muted">{f20.hitCount}/{f20.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f30.hitRate)}</span><div className="muted">{f30.hitCount}/{f30.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f50.hitRate)}</span><div className="muted">{f50.hitCount}/{f50.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f100.hitRate)}</span><div className="muted">{f100.hitCount}/{f100.testedCount} 冻结</div></td>
            <td>{f50.maxMiss}<div className="muted">当前 {f20.currentMiss}</div></td>
            <td><div className="dots">{f20.testedCount?frozenRecords.slice(0,20).reverse().map(r=>{
              const row=r.tail?.find(x=>x.id===s.id)
              return <span key={r.expect} className={row?.hit?'dot hit':'dot'} title={`第${r.expect}期｜${row?.hit?'中':'未中'}｜冻结`}/>
            }):null}</div></td>
          </tr>})}</tbody>
        </table></div>
      ...
      <section className="card">
        ...
        <div className="scroll"><table>
          ...
          <tbody>{hRanking.map((s,i)=>{
            const f20=frozenStat(frozenRecords,'head',s.id,20)
            const f30=frozenStat(frozenRecords,'head',s.id,30)
            const f50=frozenStat(frozenRecords,'head',s.id,50)
            const f100=frozenStat(frozenRecords,'head',s.id,100)
            return <tr key={s.id}>
            <td className="rank">{i+1}</td>
            <td><strong>{s.name}</strong></td>
            <td className="formula"><strong>{s.label}</strong><div className="muted">{s.desc}</div></td>
            ...
            <td><span className={f20.hitRate>=82?'rate good':f20.hitRate>=78?'rate mid':'rate low'}>{pct(f20.hitRate)}</span><div className="muted">{f20.hitCount}/{f20.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f30.hitRate)}</span><div className="muted">{f30.hitCount}/{f30.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f50.hitRate)}</span><div className="muted">{f50.hitCount}/{f50.testedCount} 冻结</div></td>
            <td><span className="rate">{pct(f100.hitRate)}</span><div className="muted">{f100.hitCount}/{f100.testedCount} 冻结</div></td>
            <td>{f50.maxMiss}<div className="muted">当前 {f20.currentMiss}</div></td>
            <td><div className="dots">{f20.testedCount?frozenRecords.slice(0,20).reverse().map(r=>{
              const row=r.head?.find(x=>x.id===s.id)
              return <span key={r.expect} className={row?.hit?'dot hit':'dot'} title={`第${r.expect}期｜${row?.hit?'中':'未中'}｜冻结`}/>
            }):null}</div></td>
          </tr>})}</tbody>
        </table></div>
